...
## ADR-025 — Project/site visibility scoping uses an explicit migration flag, not inference (V4.3)
**Context:** "Users only see assigned projects" requires filtering `GET /api/projects`/`GET /api/sites` by `assigned_project_ids` — a real, previously-deferred feature (ADR-022 explicitly named this "the real future expansion"). But `assigned_project_ids` already existed since Sprint 4.1 and defaults to `[]` when absent — if scoping were applied to every account, every Sprint 1-4.2 account (which has never had a real assignment, since assignment as a concept didn't functionally gate anything before now) would suddenly see zero projects, breaking capture/timeline/ops for every existing user. **Decision:** add a new, explicit `scope_projects: bool` field, defaulting to `false` (unrestricted — today's exact behaviour) for every account. Only `register_user()` (Sign Up) sets it `true`; `upsert_user()` (plain login, unchanged) never touches it. Filtering logic checks this flag (and unconditionally exempts `role == "management"`) rather than trying to infer "is this a legacy account" from other fields. **Alternatives:**
  - Infer "new" vs "legacy" from `approval_status` presence (every pre-4.1 account has it absent; every Sign-Up-created account has it explicitly set) → rejected: fragile — an admin manually approving/rejecting an old account (rare but possible) would flip that inference with a surprising side effect (suddenly scoping an account nobody intended to scope). An explicit, purpose-built flag has no such edge case.
  - Scope every account immediately and pre-populate `assigned_project_ids` with "all current projects" for existing users as a one-time migration script → rejected: this project has no migration-script infrastructure (every prior Sprint 4.x field addition has been read-time-default, zero-script), and a snapshot-at-migration-time assignment would silently diverge from "sees everything" the moment a new project is created post-migration — worse than just leaving legacy accounts unrestricted.
Date: V4.3.

## ADR-026 — Workspace is a new explicit field, validated against role via a compatibility map (V4.3)
**Context:** "Admin can assign Workspace (Client, Supervisor, Project Manager, Admin)" as an action independent from "Permission Level" (role) and "Project(s)". Until this sprint, workspace was purely derived (`DEFAULT_VIEW_ROLE_FOR[role]`, one-to-one except `coordinator`, which always resolved to `pm` — `client` was explicitly unreachable per ADR-020, since nothing distinguished a "client" coordinator from a "PM" coordinator). **Decision:** add `workspace` as a new, explicit, optional field. `completeLoginRouting()` prefers it when present, falling back to the exact pre-existing derivation when absent — full backward compatibility in one line. Assignment is validated against `WORKSPACE_ROLE_MAP` (mirrored on both backend and frontend, backend authoritative): `supervisor→{supervisor}`, `coordinator→{client,pm}`, `management→{admin}` — the same pairing `DEFAULT_VIEW_ROLE_FOR` already encoded, just exposed as a set instead of a single default. Changing an account's role clears a now-incompatible stored workspace rather than leaving an inconsistent combination in place. **Alternatives:**
  - Let admin set any workspace regardless of role → rejected: would allow workspace="admin" on a non-management account, showing Admin navigation while every admin-gated backend call still 403s — a confusing, broken-looking state for no benefit.
  - Auto-set role when workspace is assigned (and vice versa) → rejected: two independent admin actions (Permission Level, Workspace) is what the brief asks for; auto-coupling them removes the admin's ability to, e.g., approve a `management`-role user who should nonetheless see the `admin` workspace only after a second explicit step — the validation-not-coupling approach keeps both fully independent while still preventing invalid combinations.
Date: V4.3.

## ADR-027 — Workflow Templates are `knowledge_items`, not a new collection (V5)
**Context:** Sprint 5 needed a way to represent named Workflow Templates (Villa, Residential, Commercial, Interior, Renovation) that reference an ordered set of Activity Library items. **Decision:** add `workflow_template` as a sixth value to `knowledge_items.TYPES`, and represent a template's activity list as more entries in the same generic `relationships[]` array (`type="includes_activity"`, `metadata.order` for sequence) — not a new field, not a new mechanism. This gets CRUD, search, soft-archive, versioning, and admin-only gating entirely for free from Sprint 4's existing engine; `routes/knowledge.py` needed zero new endpoints. "Templates reference Activity Library items only" is satisfied as a side effect of `add_relationship()`'s existing target-must-exist check, not a new validation rule. **Alternatives:**
  - A dedicated `workflow_templates` collection with an `activity_ids: list[str]` field → rejected: duplicates the CRUD/search/archive/versioning machinery `knowledge_items` already provides, for a concept (a named, versioned, admin-curated master-data record) that is structurally identical to every other Knowledge Core type. This is precisely the ADR-014 reasoning (why `knowledge_items` is one collection, not five) extended by one more type, not a new argument.
Date: V5.

## ADR-028 — Project workflow instances are a new, separate collection from `knowledge_items` (V5)
**Context:** Generating a project's workflow needs many independent, mutable, per-project copies of Activity Library data, each tracking its own status. This is a fundamentally different kind of data from `knowledge_items` (global, admin-curated, versioned reference data — one canonical "Concrete Slab Pour" shared across every project). **Decision:** a new `workflow_activities` collection, denormalizing each activity's Sprint 5 fields (`name`, `trade`, `unit`, `default_duration_days`, `requires_inspection`) onto the instance at generation time, keeping `knowledge_activity_id` only for traceability — not a live reference. Dependency links are translated from the source activities' `depends_on` relationships into concrete sibling-instance ids within the same project at generation time, then tracked independently per project (`depends_on_activity_ids`). **Alternatives:**
  - Reuse `knowledge_items` for project-scoped instances too (e.g. a `project_id` field on a copy of the activity) → rejected: conflates global reference data with mutable, per-project instance state in the same collection Sprint 4 carefully kept generic and reference-only; would also force every Knowledge Core list/search/version/archive operation to reason about a `project_id` dimension it has no other reason to know about.
  - Live-reference the Activity Library (workflow_activities just stores `knowledge_activity_id`, always reads through to the current activity for name/trade/unit/etc.) → rejected: a project's generated workflow must not silently change shape if someone edits the Activity Library afterward — the same reasoning as `assigned_to_user_name` being denormalized alongside `assigned_to_user_id` everywhere else in Atlas.
Date: V5.

...
def test_total_users_reflects_new_accounts(admin):
    """FAC-03 P0 fix: login no longer creates accounts - /auth/register
    is the only account-creation path now, so that's what this test
    exercises to verify total_users reflects new accounts."""
    before = requests.get(f"{API}/admin/system-info", headers=admin["headers"], timeout=20).json()
    requests.post(f"{API}/auth/register", json={"phone": "9888800002", "name": "V42 New Signup"}, timeout=20)
    after = requests.get(f"{API}/admin/system-info", headers=admin["headers"], timeout=20).json()

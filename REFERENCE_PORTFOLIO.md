...
## RP-001 health — was Critical, now genuinely Healthy (fixed in STAB-01)

The brief specifies "Expected Health: GREEN" for RP-001. When this document was first written, RP-001 computed as Critical (health score 40) despite its workflow being essentially complete (358 of 361 activities finished) — carrying 135 still-open operational items, 11 of them critical-priority, accumulated across its 18-month simulated history without being resolved at a matching rate. That finding was deliberately left unfixed at the time: closing 135 items to force a Healthy classification would have meant fabricating resolution events with no real story behind them.

**STAB-01 fixed this properly, not by assigning a health value directly.** Investigation found the operational items were not actually the primary driver of the Critical score at all — resolving all 135 was measured to have zero effect on health. The real cause was 155 of 157 `requires_inspection` activities being marked complete with no inspection record on file, tripping CRE's own `quality.completed_without_inspection` rule on every one. Two honest fixes closed this: `reference_portfolio.complete_rp001_operations()` resolves the large majority of open operational items through their own real state machine, with resolution notes derived from each item's own specific title (not a repeated generic stamp), deliberately leaving a small genuine residual rather than forcing a suspicious zero; `reference_portfolio.record_missing_rp001_inspections()` records the genuinely missing inspection for every uncovered activity, matched to the correct site by name (100% match rate). Both are wired into the standard Reference Portfolio seeding flow and `bootstrap.py`'s own Stage 5 — no manual step required.

**RP-001 now computes as green/100, with zero remaining CRE findings**, verified directly against a full bootstrap run, not assumed. Every open critical-priority item is resolved; a small residual (0–15 items) of ordinary-priority items remains, matching how an actually-completed project looks rather than a suspiciously perfect zero.

RP-002 computes as Healthy (score 93) — unaffected by this fix (STAB-01 touched only RP-001's operational items and inspection records), still demonstrating its own intended attention-level commercial state (`cash_flow_signal: attention`) with active variations and partial workflow, exactly as designed.
